...
class RegexBuilder(QDialog):
    ...
    def make_regex(self):
        ...
        for x, y in self.regex_indices.data[::-1]:  # iterate list in reverse, so our indices stay correct while modifying the string
            if y is 'd':
                self.text = self.text[:x] + dyn_regex + self.text[previous:]
                previous = x
            elif y is 'e':
                ep_digits = previous-x
                self.text = self.text[:x] + '(' + ep_regex*ep_digits + ')' + self.text[previous:]
                previous = x
            elif y is 's':
                escaped_text_segment = self.text[x:previous]
                for c in qa2_util.CHARSET_REGEX_ESCAPES:
                    if c in escaped_text_segment:
                        escaped_text_segment = escaped_text_segment.replace(c, "\\" + c)
                self.text = self.text[:x] + escaped_text_segment + self.text[previous:]
                previous = x
            qa2_util.debug(self.text, level=1)
        self.text = "^" + self.text
